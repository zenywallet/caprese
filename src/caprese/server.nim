    ...
    when cfg.sslLib == BearSSL:
      if client.sc.isNil:
        ...
        when defined(linux):
          ...
          EV_SET(addr client.ev, client.sock.uint, EVFILT_WRITE, EV_ENABLE, 0, 0, client.ev.udata)
          release(client.spinLock)
...
template serverLib(cfg: Config) {.dirty.} =
  ...
  proc appRoutesSend(ctx: ServerThreadCtx) {.thread.} =
    ...
    proc taskCallback(task: ClientTask): bool =
      ...
      result = (lastSendErr == SendResult.Success or lastSendErr == SendResult.Pending)
        ...
        if client.dirty == ClientDirtyNone:
          if client.appShift and client.sendCurSize == 0:
            ...
            when defined(linux):
              ...
              EV_SET(addr client.ev, client.sock.uint, EVFILT_WRITE, EV_DISABLE, 0, 0, client.ev.udata)
          client.threadId = 0
          release(client.spinLock)
                    ...
                    if nextParse >= 0:
                      let retMain = when requestMethod == RequestMethod.GET: routesMain(ctx, client)
                          ...
                          if nextParse > ctx.parseSize:
                            if nextParse > cfg.recvBufExpandBreakSize:
                              client.close()
                              return
                            else:
                              client.addRecvBuf(ctx.pRecvBuf, ctx.parseSize)
...
proc searchServerNode*(searchNode: NimNode) =
  for n in searchNode:
    searchServerNode(n)
  ...
  proc searchAddServer(searchNode: NimNode) =
    for n in searchNode:
      ...
      if searchNode.kind == nnkCall and eqIdent(n, "addServer"):
        searchServerNode(searchNode[searchNode.len - 1])
